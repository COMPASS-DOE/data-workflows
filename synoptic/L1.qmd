...
f <- function(dir_name, dirs_to_process, out_dir) {
    ...
    # Make sure Plot and TIMESTAMP columns are on the left
    ...
    dat <- dat[setdiff(colnames(dat), 
                       c("Site", "Logger", "Table", "design_link",
...
var_md <- read.csv(file.path(params$METADATA_ROOT, params$METADATA_VARS_TABLE))
    ...
    for(f in files) {
        ...
        if(!identical(sort(col_md$Column), sort(file_headers))) {
            stop("File ", basename(f), 
                 ...
                 "\n--> Fix this by updating ", params$METADATA_VARS_TABLE)
        }
