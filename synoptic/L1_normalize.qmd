...
troll <- read_csv(file.path(params$METADATA_ROOT, params$OOS, "troll_maintenance.csv"),
                  col_types = "ccccccccc")
oos_troll <- prep_troll_oos_table(troll) # in out-of-service.R
# Read the EXO out-of-service table
exo <- read_csv(file.path(params$METADATA_ROOT, params$OOS, "exo_log.csv"),
                col_types = "cdDDDDcc")
oos_exo <- prep_exo_oos_table(exo) # in out-of-service.R
...
f <- function(fn, out_dir, design_table) {
    ...
    message("\tRemoved ", nrow(dat_nodupe) - nrow(dat), " NA data value rows")
    ...
    # It turns out that the out of service check is SUPER expensive (50% of 
    # driver runtime). Since right now it's only applicable to Aquatroll 
    # and EXO, check and skip if possible
    if(grepl("WaterLevel", dat$Table[1])) {
        dat$F_OOS <- as.integer(oos(oos_troll, dat))
        message("\tAdding ", sum(dat$F_OOS), " OOS flags for WaterLevel")
    } else if(grepl("ExoTable", dat$Table[1])) {
        dat$F_OOS <- as.integer(oos(oos_exo, dat))
        message("\tAdding ", sum(dat$F_OOS), " OOS flags for ExoTable")
    } else {
        dat$F_OOS <- 0L
    }
