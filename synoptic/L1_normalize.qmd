...
dt_ex$design_link <- with(dt_ex, 
                          paste(Site, Plot, instrument, which, individual, research_name, sep = "-"))
...
f <- function(fn, out_dir, design_table) {
    ...
    message("\tFiltering out ", smry$no_research_name, " empty no_research_name rows")
    dat <- subset(dat, !is.na(dat$research_name))
    
    # If no rows left, note this fact and move on
    if(!nrow(dat)) {
        message("\tNo design links left; nothing to process")
