...
```{r}
# do unit conversion for data frame and unit conversion table:
# isolate the various research_name entries one by one, find corresponding
# conversion string, evaluate it against the whole `value` data column
do_unit_conversion <- function(dat, ut) {
    dat_conv <- list()
    for(rn in unique(dat$research_name)) {
        ...
        message("\t\tUC: ", rn, " n=", nrow(d), ", conv=", conv)
...
f <- function(dir_name, dirs_to_process, out_dir) {
    ...
    # Round timestamps
    dat$TIMESTAMP_R <- round_date(dat$TIMESTAMP, params$timestamp_round)
    ...
    dat_mrg %>% 
        ...
        mutate(Dir = basename(dir_name), 
               Note = "") %>%
        select(Dir, Site, Plot, Vars, TS_min, TS_max, Note) ->
        smry
    ...
    lefts <- c("Site", "Plot", "Tree", "TIMESTAMP_R", "research_name", "value")
