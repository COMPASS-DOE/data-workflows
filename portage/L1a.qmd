...
# For compactness, the plot table may have expansions
PLOT_TABLE <- file.path(params$DATA_ROOT, params$PLOT_TABLE)
pt <- read_csv(PLOT_TABLE, col_types = "cccc")
pt_ex <- compasstools::expand_df(pt)
...
Plot table "`r PLOT_TABLE`" has `r nrow(pt)` rows, `r nrow(pt_ex)` after expansion.
...
f <- function(dir_name, dirs_to_process, out_dir) {
    ...
    dat$TIMESTAMP_raw <- dat$TIMESTAMP
    ...
    dat %>% 
        ...
        select(-Table, -loggernet_variable, -design_link) ->
        dat_mrg
