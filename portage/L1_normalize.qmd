...
params:
  ...
  UNITS_TABLE: "units_table.csv"
  BOUNDS_TABLE: "bounds_table.csv"
  logfile: ""
date: now
...
dt <- read_csv(DESIGN_TABLE, col_types = "cccccc")
...
UNITS_TABLE <- file.path(params$DATA_ROOT, params$UNITS_TABLE)
ut <- read_csv(UNITS_TABLE, col_types = "ccccccc")
ut <- filter(ut, !is.na(research_name))

# Read the bounds table (not everything needs an entry)
BOUNDS_TABLE <- file.path(params$DATA_ROOT, params$BOUNDS_TABLE)
read_csv(BOUNDS_TABLE, col_types = "cccccc") %>% 
...
Units table is "`r UNITS_TABLE`" and has `r nrow(ut)` rows with conversion strings.

Bounds table is "`r BOUNDS_TABLE`" and has `r nrow(bt)` rows with bounds.
