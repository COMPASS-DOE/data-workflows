...
params:
  ...
  METADATA_TABLE: "metadata_vars.csv"
  debug: false
...
METADATA_TABLE <- file.path(params$DATA_ROOT, params$METADATA_TABLE)
mt <- read_csv(METADATA_TABLE, col_types = "ccccccddc")
...
Variable metadata table is "`r METADATA_TABLE`" and has `r nrow(mt)` rows.
...
f <- function(fn, out_dir, design_table) {
    ...
    cat(Sys.time(), " Processing ", basename(fn), "\n")
    ...
    cat("\tPivoted data has ", nrow(dat_long), " rows\n")
    ...
    dat_long <- subset(dat_long, !is.na(dat_long$design_link))
    ...
    dat_long %>% 
        ...
        mutate(OOB = value < low_bound | value > high_bound) ->
        dat_long
    
    # Remove unneeded columns unless needed for debugging
    if(!params$debug) {
        message("\tDropping bounds columns")
        dat_long <- select(dat_long, -low_bound, -high_bound)
    }
    
    write_to_folders(dat_long, 
                     root_dir = out_dir, 
