...
f <- function(fn, out_dir, design_table) {
    ...
    # Write out a debug table with all the unique site-logger-table-variables we see 
    if(params$debug_design_links) {
        fn <- "designlinks.csv"
        dat_long %>% 
            select(Site, Logger, Table, loggernet_variable, design_link) %>% 
            distinct() %>% 
            readr::write_csv(fn, append = file.exists(fn))
    }
