...
L1 metadata directory is `r params$L1_METADATA`.
...
for(dd in data_dirs) {
    ...
    for(f in files) {
        ...
        if(!identical(sort(col_md$Column), sort(file_headers))) {
            stop("File ", f, " headers don't match metadata in L1_column_metadata.csv")
        }
    ...
    # Optionally, there can be a <site>_<year>.txt metadata file, too, providing
    # year-specific information
    site_year_md_file <- file.path(params$DATA_ROOT, 
                                   params$L1_METADATA, 
                                   paste0(dd, ".txt"))
    if(file.exists(site_year_md_file)) {
        message("\tFound ", basename(site_year_md_file))
        site_md_for_insert <- c(site_md_for_insert, readLines(site_year_md_file))
    }
